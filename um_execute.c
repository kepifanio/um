...
void execute_um(segment_t program)
{
    ...
    uint32_t word;
    while (run) {
        word = segmented_load(memory, 0, memory->program_counter);
        ...
        if (op_code == 13) {
            ...
            if (op_code == 0) {
                ...
                memory->eight_regs[ra] = segmented_load(memory,
...
void load_instruction(mem_t memory, uint32_t rb, uint32_t rc)
{
    if (memory->eight_regs[rb] != 0) {
        segment_t instr = Seq_get(memory->segments, memory->eight_regs[rb]);
        segment_t new_segment = init_seg((uint32_t)Seq_length(instr));
        for (uint32_t i = 0; i < (uint32_t)Seq_length(instr); i++) {
            store_instruction(get_segment(i, instr), new_segment);
        }
        Seq_put(memory->segments, 0, new_segment);
    }
